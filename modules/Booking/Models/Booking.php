...
class Booking extends BaseModel
{
    ...
    {
        $all = get_bookable_services();
        if ($this->object_model and $this->object_model == "hotel" and !empty($all[$this->object_model])) {
            $hotel = $this->hasOne($all[$this->object_model], 'id', 'object_id')->first();
            if ($hotel){
                $hotels =  Hotel::where('location_id',$hotel->location_id)->get();
                return $hotels;
            }
            return null;
        }
    ...
    $vendors = User::query()->orderBy('id','desc')->role('vendor')->count();
    $rooms_number = HotelRoom::where([['status','publish']])->sum('number');
//   $available =  HotelRoomDate::where('end_date','=',date('Y-m-d'))->sum('number');
  $booked_rooms= HotelRoomBooking::whereDate('start_date', '<=', date("Y-m-d"))
                                    ->whereDate('end_date', '>=', date("Y-m-d"))->sum('number');

  $available = $rooms_number-$booked_rooms;
    // $rooms_number= HotelRoom::select(DB::raw('sum(number) as total'))->get();
//    dd($rooms_number);
           // count
           $booked_packages_count = RoomPackageBooking::where([['is_paid','1'],['updated_at','>=', Carbon::now()->subMonth()]])->count();
           $booked_hotel_count = Booking::where([['is_paid','1'],['updated_at','>=', Carbon::now()->subMonth()]])->count();
           // total
           $booked_packages_total = RoomPackageBooking::where([['is_paid','1'],['updated_at','>=', Carbon::now()->subMonth()]])->pluck('total')->sum();
           $booked_hotel_total = Booking::where([['is_paid','1'],['updated_at','>=', Carbon::now()->subMonth()]])->pluck('total')->sum();
           $balance_before =  $booked_packages_total + $booked_hotel_total; //balance before site percentage
            $site_balance = $balance_before * .1;
        // $total_booking = parent::whereNotIn('status',static::$notAcceptedStatus)->count('id');
        $total_booking = $booked_packages_count + $booked_hotel_count; //count of all bookings
        ...
        $res[] = [
            ...
            'size_md'=>3,
            'title'  => __("Total"),
            'amount' => format_money_main($balance_before),
        ...
        $res[] = [
            ...
            'size_md'=>3,
            'title'  => __("Earning"),
            'amount' => format_money_main($site_balance),
        ...
        $res[] = [
            ...
            'class'  => 'info',
            'icon'   => 'icon ion-ios-pricetags',
            'href'=>'admin/module/user?role=vendor&s=',
        ];
        $res[] = [
            ...
            'title'  => __("Rooms"),
            'amount' => $available,
            'desc'   => __("Available Number Of Rooms"),
            'class'  => 'success',
...
//        $list_booking->where('status','!=','draft');
    ...
    public static function getTopCardsReportForVendor($user_id,$balance_before2,$total_booking2,$withdrow_date)
    {
        ...
        $total_money2 =VendorRequest::where('user_id',$user_id)->pluck('balance')->first();
//   dd($total_money);
        // $total_booking = parent::whereNotIn('status',static::$notAcceptedStatus)->where("vendor_id", $user_id)->count('id');
        // count
        $booked_packages_count = RoomPackageBooking::where([['vendor_id',$user_id],['is_paid','1'],['updated_at','>',$withdrow_date]])->count();
        $booked_hotel_count = Booking::where([['vendor_id',$user_id],['is_paid','1'],['updated_at','>',$withdrow_date]])->count();
        // total
        $booked_packages_total = RoomPackageBooking::where([['vendor_id',$user_id],['is_paid','1'],['updated_at','>',$withdrow_date]])->pluck('total')->sum();
        $booked_hotel_total = Booking::where([['vendor_id',$user_id],['is_paid','1'],['updated_at','>',$withdrow_date]])->pluck('total')->sum();
        if($total_money2 != 0){
            $balance_before =  $booked_packages_total + $booked_hotel_total; //balance before site percentage

            $booked_packages_total = RoomPackageBooking::where([['vendor_id',$user_id],['is_paid','1'],['updated_at']])->pluck('total')->sum();
            $booked_hotel_total = Booking::where([['vendor_id',$user_id],['is_paid','1']])->pluck('total')->sum();
        ...
        // ];
        $res[] = [
            'title'  => __("Net profit Now"),
            'amount' => format_money_main($total_money2 ?? 0),
            'desc'   => __("Your balance after deducting the 10% of the site"),
        ];
        $res[] = [
            'title'  => __("total Earning Now"),
            'amount' => format_money_main($balance_before2 ?? 0),
            'desc'   => __("Your balance Befor deducting the percentage of the site"),
        ];

        $res[] = [
            'title'  => __("No of Bookings"),
            'amount' => $total_booking2 ,
            'desc'   => __("No. of All Bookings"),
        ];
        // $res[] = [
        //     'title'  => __("Bookings"),
        //     'amount' => $total_booking,
    ...
    public static function getEarningChartDataForVendor($from, $to, $user_id)
    {
        ...
        // $booked_packages_total = RoomPackageBooking::where([['vendor_id',$user_id],['is_paid','1']])->pluck('total')->sum();
        // $booked_hotel_total = Booking::where([['vendor_id',$user_id],['is_paid','1']])->pluck('total')->sum();
        // $balance_before =  $booked_packages_total + $booked_hotel_total; //balance before site percentage
        // dd($data);
        return $data;
