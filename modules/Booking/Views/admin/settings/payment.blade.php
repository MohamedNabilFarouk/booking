...
<div class="row">
    ...
    <div class="col-md-8">
        @php
            ...
            // $all['paymob']="Modules\Booking\Gateways\PayMobGateway1";
             $all = App\PaymentGateway::all();
        ...
        @foreach($all as $k=>$g)
        <div class="panel">
            <div class="panel-title"><strong>{{$g['name']}}</strong>
                <span data-toggle="collapse" href="#gateway_{{$k}}" class="panel-toggle btn btn-sm" ><i class="fa fa-chevron-down"></i></span>
            </div>
            <div class="panel-body collapse" id="gateway_{{$k}}">
               <form action='{{url('updateGateway',$g->id)}}' method='post' id='form_{{$k}}'>
                @csrf
                @method('put')
                <label>@if($g->status=='0') Enable @else Disbale @endif {{$g->name}}</label>
                <input type='checkbox' name='status'  onChange='submit();'  value='1' >
                {{-- <button type='submit' class='btn btn-succcess' id='btn_{{$k}}'>save</button> --}}
            </form>
            </div>
        </div>
        <script>
...
$(document).ready(function(){
    $("#btn_{{$k}}").click(function(){        
        $("#form_{{$k}}").submit(); // Submit the form
    });
});
