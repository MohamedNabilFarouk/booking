...
@if(isset($gateway)&&($gateway->name =='PayMob'))
    <form action='{{url('payMobPayment')}}' method='get'>
        @elseif(isset($gateway)&&($gateway->name =='Fawry'))
            {{-- <form action='{{url('opay')}}' method='get'> --}}
            <form action='{{url('fawryPayment')}}' method='get'>
                ...
                <div class="form-group">
                    ...
                    <div class="form-actions">
                        ...
                        @endguest
                        @auth
                            {{-- <button class="btn btn-danger" @if(empty($gateway)) @click="doCheckout" @endif >{{__('Submit')}}
                                <i class="fa fa-spin fa-spinner" v-show="onSubmit"></i>
                            </button> --}}
                            @if($booking->is_confirmed == 1)
                                <input type="submit" class="btn btn-danger" value="Pay By OPAY" formaction="{{url('opay')}}">
                            @endif
