                ...
                <form action="" class="bravo-form-item">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th width="80px"> {{ __('Reviews')}}</th>
                            <th width="220px"> {{ __('Rooms')}}</th>
                        ...
                        @if(count($rows) > 0)
                            @foreach($rows as $index => $row)
                                <tr class="{{$row->status}}">
                                    ...
                                    <td>
                                        <span class="badge d-block">{{__('Rooms Kind')}}: {{ count($row->allRoom)}}</span>
                                        <span class="badge d-block">{{__('Rooms Count')}}: {{$row->allRoom->sum('number') }}</span>
                                        <span class="badge d-block">{{__('Publish')}}: {{ count($row->rooms) }} </span>
                                        <span class="badge d-block">{{__('Pending')}}: {{ count($row->pendingRooms) }}</span>
                                    </td>
