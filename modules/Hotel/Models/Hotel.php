...
class Hotel extends Bookable
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $booking->is_confirmed = $this->auto_confirmed;
        ...
        if (Auth::user()){
            ...
            $data = [
                ...
                'link'    => get_link_detail_services($booking->type, $booking->id, 'index'),
                'type'    => $booking->type,
                'message' => $booking->is_confirmed == 1 ? " تم عمل طلب حجز جديد من قبل ". Auth::user()->phone . " " . Auth::user()->display_name . " " : " تم عمل طلب حجز جديد من قبل " . Auth::user()->phone . " " . Auth::user()->display_name . " يحتاج للتأكيد "
            ];
            // notify to admin
    ...
    {
        return $this->hasMany($this->roomClass, 'parent_id')
    ...
    {
        return $this->hasMany($this->roomClass, 'parent_id')
            ->where('status', "pending")->with("translations");
    }
...
//         $ip = '41.233.122.119'; //test for local
        ...
        foreach ($rooms as $room) {
            if ($room->isAvailableAt($filters)) {
                // **************
                // $p = RoomPrices::where([['ip',$data->countryCode],['room_id',$room->id]])->first();
                // if(isset($p->price)){
                // // Session::put('room_price', $p->price);
                // $price = $p->price;
                // }else{
                //   $price = $room->price;
                // }
                ...
                $adult_guest = request()->input('adults');
                $children_guest = request()->input('children');
                ...
                $res[] = [
                    ...
                    'adults'            => $room->adults ?  $room->adults : '',
                    'children_html'   => $room->children ? 'x' . $room->children : '',
                    'children'   => $room->children ?  $room->children : '',
                    'ages'      =>request()->input('age'),
                    'rooms_no' =>   $adult_guest >= $children_guest ?  ceil(( $adult_guest / $room_adults)):  ceil(($adult_guest/$room_adults) + ($children_guest - $room_children)/($room_children+$room_adults)),
                    'nights_no'=>    $room->tmp_nights,
                    'number_selected' => 0,
                    // 'number'          => (int)$room->number ?? 0,
                    'number'          => (int)$room->tmp_number ?? 0 , //original
                    'fake_number' => (int)$room->number ?? 0,
                    ...
                    //  'price_html'      => format_money($price) . '<span class="unit">/' . ($room->tmp_nights ? __(':count nights', ['count' => $room->tmp_nights]) : __(":count night", ['count' => $room->tmp_nights])) . '</span>',
                    // 'price_text'      => format_money($price) . '/' . ($room->tmp_nights ? __(':count nights', ['count' => $room->tmp_nights]) : __(":count night", ['count' => $room->tmp_nights])),
                    'terms'           => $terms,
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
    ...
    public function dataForApi($forSingle = false){
        ...
        $data['translation']= $this->translations;
        return $data;
    }
    ...
    public function getPriceAttribute($value){
        $room = $this->roomClass::where([['parent_id', '=', $this->id]])
        ...
        if (!empty($room)){
            if (config()->get('app.country') == "EG"){
                ...
                $room_price = \Modules\Hotel\Models\RoomPrices::where([
                    ['ip', '=', config()->get('app.country')],
                    ['room_id', '=', $room->id]
                ])->select('price')->first();
                $p = !empty($room_price) ? $room_price->price : $room->price;
                return $p;
            }
        }else{
            return !empty($this->price) ? $this->price : ceil($value);
        }
    }
}
