...
//        $ip = '41.233.122.119'; //test for local
        ...
        $p = RoomPrices::where([['ip','=',$data->countryCode],['room_id','=',$this->id]])->first();
        if(isset($p->price)){
            $price = $p->price;
        }else{
            $price = $this->price;
...
//                {
    ...
    {
        return $this->roomBookingClass::query()->where('room_id',$this->id)
            ->active()->inRange($from,$to)->get(['bravo_hotel_room_bookings.*']);
    }
    ...
    public function roomPackage(){
        return $this->hasMany(RoomPackage::class , 'room_id', 'id');
    }
    ...
    public function countryPrices(){
        return $this->hasMany(RoomPrices::class, 'room_id', 'id' )
            ->groupBy('price')->with('ips');
    }

    public function getPriceAttribute($value){
        $ppp = !empty($this->price) ? $this->price : ceil($value);
        if (config()->get('app.country') == "EG"){
            ...
            $p = !empty($room_price) ? $room_price->price : $ppp;
            return $p;
        }
    }
}
