...
class Tour extends Bookable
{
    ...
    //         //   $p = TourPrices::where([['ip',$data->countryCode],['tour_id',$this->id]])->first();  
    //         if(isset(Auth::user()->country)){  
    //         if((Auth::user()->country == 'Egypt') || (Auth::user()->country == 'EG') || ($this->create_user == (Auth::user()->id)) ){
    //           $value = $value;
              
    //        }else{
    //         // dd(Auth::user()->country);
    //             $value = $this->attributes['other_price'];  
    ...
    public function getPriceAttribute($value){
        if (config()->get('app.country') !== "EG" && !empty($this->other_price)){
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        $data['duration'] = duration_format($this->duration);
            ...
            $data['other_price'] = ($this->other_price == null)? $this->price: $this->other_price;  
            // $var > 2 ? true : false
            $data['is_tour']=true;  
