...
class UserController extends FrontendController
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $user_id = Auth::id();
        // balance of vendor 
        $trans = VendorTransaction::where('vendor_id',$user_id)->orderBy('updated_at', 'desc')->first();
        if(isset($trans)){
                $withdrow_date= $trans->updated_at;
                // dd($withdrow_date);
                $booked_packages_total = RoomPackageBooking::where([['vendor_id',Auth::id()],['is_paid','1'],['updated_at','>',$withdrow_date]])->pluck('total')->sum();
                $booked_hotel_total = Booking::where([['vendor_id',Auth::id()],['is_paid','1'],['updated_at','>',$withdrow_date]])->pluck('total')->sum();
                //  dd($booked_hotel_total);

                 // for total number
            $packages_total = RoomPackageBooking::where([['vendor_id',Auth::id()],['is_paid','1'],['updated_at','>',$withdrow_date]])->count();
            $hotel_total = Booking::where([['vendor_id',Auth::id()],['is_paid','1'],['updated_at','>',$withdrow_date]])->count();
            
            $total_booking2 = $packages_total + $hotel_total;
        }else{
            // dd('here');
            ...
            // for total number
            $packages_total = RoomPackageBooking::where([['vendor_id',Auth::id()],['is_paid','1']])->count();
            $hotel_total = Booking::where([['vendor_id',Auth::id()],['is_paid','1']])->count();
         ...
         $balance_before2 =  $booked_packages_total + $booked_hotel_total;
        $balance_after = $balance_before2 - ($balance_before2 * .1);
        ...
        //   $request->start = Carbon::now()->subMonth()->toDateTimeString();
           $transactions = VendorTransaction::where('vendor_id',$user_id)->orderBy('updated_at', 'desc')->paginate(10);
           }else{
            $transactions= VendorTransaction::where('vendor_id',$user_id)->whereBetween('withdrawal_date',array($request->start , $request->end))->orderBy('updated_at', 'desc')->paginate(10);
           }
       
           
        // $ss = DB::table('room_package_booking')
        // ->where([['vendor_id',$user_id],['is_paid','1'],['updated_at','>=',Carbon::now()->subMonth()]])
        // ->select([
        //     DB::raw('sum(total) as daily'),
        ...
        // ->get()
        // ->keyBy('date');
        // $collection = $dd->mergeRecursive($ss); 
// dd($collection);

//            $booked_packages_total = RoomPackageBooking::where([['vendor_id',$user_id],['is_paid','1'],['updated_at','>=',Carbon::now()->subMonth()]])->select(['total','updated_at'])->get();
...
//            $earning_chart = 0;
        ...
        // end balance
        $data = [
            'cards_report'       => Booking::getTopCardsReportForVendor($user_id,$balance_before2,$total_booking2,$withdrow_date),
    ...
    {
        $user_id = Auth::id();
        $package_booking_history = RoomPackageBooking::where('user_id',$user_id)->orderBy('id','DESC')->get() ;
        $data = [
            'bookings'    => Booking::getBookingHistory($request->input('status'), $user_id),
            'package'      =>$package_booking_history,
    ...
    public function upgradeVendor(Request $request){
        ...
        if(!empty($vendorRequest)){
            if($request->has('tax_card')){
                $vendorRequest['tax_card'] = $request->tax_card;
            }
            if($request->has('commercial_record')){
            $vendorRequest['commercial_record'] = $request->commercial_record;
            }
            if($request->has('bank_account')){
            $vendorRequest['bank_account']= $request->bank_account;
            }
            if($request->has('bank_name')){
                $vendorRequest['bank_name']= $request->bank_name;
                }
            $vendorRequest->save();
        ...
        // if ($request -> has('commercial_record')) {
        //     $image = $this -> saveImages($request -> tax_card, 'images/commercial_record');
    ...
    public function getVendorData(){
        $user= Auth::user();
        return view('User::frontend.profile.vendorData',compact('user'));
    }
