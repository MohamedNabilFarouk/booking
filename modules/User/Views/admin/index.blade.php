                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge d-block">
                                        {{__("Hotels Count")}}: {{count($row->hotels)}}
                                    </span>
                                    @foreach($row->hotels as $index=>$hotel)
                                        <span class="badge d-block">
                                            {{__("Hotel")}}{{$index + 1}}:
                                            <a href="{{route('hotel.admin.edit',['id'=>$hotel->id])}}">{{$hotel->title}}</a>
                                        </span>
                                    @endforeach
                                </td>
                                <td class="d-flex justify-content-between" style="width: 160px;">
                                    <span class="badge d-block">{{format_money_main($row->vendorRequest->balance ??'')}}</span>

                                    @if(count($row->transactionsHistory) > 0)
                                    <button type="button" class="btn btn-sm btn-info" data-toggle="modal" data-target="#History{{$row->id}}">
                                        <sup>{{count($row->transactionsHistory)}}</sup>
                                        {{__('History')}}
                                    </button>
                                    @endif
                                    <div class="modal fade" id="History{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <table>
                                                        ...
                                                        <tbody>
                                                            @foreach($row->transactionsHistory as $index=>$h)
                                                                <tr>
                                                                    <td>{{$index + 1}}</td>
                                                                    <td>{{$h->withdrawal_amount}}</td>
                                                                    <td>{{isset($h->user) ? $h->user->name : ""}}</td>
                                                                    <td>{{display_datetime($h->created_at)}}</td>
                                                                </tr>
                                                            @endforeach
                                                        </tbody>
                                                    </table>
                                ...
                                <td>{{$row->vendorRequest->bank_account ??''}}</td>
                                <td>{{$row->vendorRequest->bank_name ??''}}</td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <!-- <a href="{{route('user.admin.wallet.addCredit',['id'=>$row->id])}}" class="dropdown-item"><i class="fa fa-plus"></i> {{__("Add Credit")}}</a> -->
                                             <a href="{{route('returnToZero',$row->id)}}" class="dropdown-item"><i class="fa fa-plus"></i> {{__("Zero account")}}</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
